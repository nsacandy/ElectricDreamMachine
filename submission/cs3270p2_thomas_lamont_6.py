"""
This script implements an ensemble prediction model combining traditional machine
learning techniques with deep learning models
...
X_train = train_data.drop(columns=['Transported', 'PassengerId', 'Name',
    'Cabin', 'Deck_A', 'Deck_B', 'Deck_C', 'Deck_D', 'Deck_E', 'Deck_F', 'Deck_G', 'Deck_T', 'Num'])
y_train = train_data['Transported'].astype(int)

X_test = test_data.drop(columns=['Transported', 'PassengerId',
    'Name', 'Cabin', 'Deck_A', 'Deck_B', 'Deck_C', 'Deck_D', 'Deck_E', 'Deck_F',
    'Deck_G', 'Deck_T', 'Num'])
...
def create_rnn_model():
    ...
    model = Sequential()
    model.add(SimpleRNN(50, input_shape=(X_train_scaled_reshaped.shape[1],
        X_train_scaled_reshaped.shape[2]), return_sequences=True))
...
def create_lstm_model():
    ...
    model = Sequential()
    model.add(LSTM(100, input_shape=(X_train_scaled_reshaped.shape[1],
        X_train_scaled_reshaped.shape[2]), return_sequences=True))
    model.add(Dropout(0.2))
...
ensemble_majority_vote = np.apply_along_axis(lambda x: np.bincount(x).argmax(),
    axis=0, arr=ensemble_preds)
...
submission_df = pd.DataFrame({'PassengerId': test_data['PassengerId'],
    'Transported': ensemble_majority_vote})
